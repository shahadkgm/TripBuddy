...
export const tripService = {
  async getAllTrips(filters: ITripFilters): Promise<PaginatedTrips> {
    const response = await api.get<ApiResponse<PaginatedTrips>>('/api/plantrips/all', {
      params: filters,
    });
  ...
  async assignGuide(tripId: string, guideId: string | null): Promise<ITrip> {
    const response = await api.patch<ApiResponse<ITrip>>(`/api/plantrips/${tripId}/guide`, {
      guideId,
    });
    return response.data.data;
  },
