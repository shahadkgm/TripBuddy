                    ...
                    <div className="relative">
                        ...
                        <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 transition-opacity duration-200 ${paginationLoading ? 'opacity-40' : 'opacity-100'}`}>
                            {requests.map((req) => (
                                <div
                                    key={req._id}
                                    onClick={() => req.tripId?._id && navigate(`/trip-details/${req.tripId._id}`, { state: { from: '/profile' } })}
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-slate-800 text-sm group-hover:text-indigo-600 transition-colors">{req.tripId?.title || 'Trip Request'}</h4>
                                            <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">{req.tripId?.destination || 'N/A'}</p>
                                        </div>
                                    </div>
                                    <div className="text-right flex flex-col items-end">
                                        <span className={`text-[9px] font-black uppercase tracking-tight px-2 py-1 rounded-full ${req.status === 'accepted' ? 'bg-emerald-100 text-emerald-600' :
                                            req.status === 'rejected' ? 'bg-rose-100 text-rose-600' :
                                                'bg-amber-100 text-amber-600'
                                            }`}>
                                            {req.status}
                                        </span>
                                        <p className="text-[9px] text-slate-400 mt-1 font-bold">sent to {(req.receiverId as any)?.name || 'Traveler'}</p>
