  ...
  const loadData = async (pageNum: number = 1) => {
    ...
    try {
      ...
      const [tripData, reqData] = await Promise.all([
        tripService.getUserTrips(userId, pageNum, LIMIT),
      ...
      {trips.length > 0 ? (
        <>
          <div className="relative">
            ...
            <div
              className={`grid grid-cols-1 gap-4 transition-opacity duration-200 ${paginationLoading ? 'opacity-40' : 'opacity-100'}`}
            >
              {trips.map(trip => {
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-4 text-xs font-medium text-slate-500">
                              <span className="flex items-center gap-1.5">
                                <Globe size={14} className="text-slate-400" /> {trip.destination}
                              </span>
                              <span className="flex items-center gap-1.5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                          ...
                          <div className="flex flex-col gap-3 justify-center">
                            <button
                              onClick={() =>
                                navigate('/expenses', {
                                  state: { tripId: trip._id, from: '/profile' },
                                })
                              }
                        ...
                        {tripRequests.length > 0 && (
                          <div className="mt-6 pt-6 border-t border-slate-100">
                            <h5 className="text-[10px] font-black uppercase tracking-widest text-amber-500 mb-3 flex items-center gap-2">
                              <UserCheck size={12} /> Pending Requests for this Trip (
                              {tripRequests.length})
                            </h5>
