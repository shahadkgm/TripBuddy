                        ...
                        return (
                            <div key={msg._id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[80%] ${isOwn ? 'order-1' : 'order-2'}`}>
                                    <div className={`p-3 rounded-2xl shadow-sm text-sm ${isOwn ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none'}`}>
                                        {!isOwn && <p className="text-[10px] font-black text-indigo-600 uppercase mb-1">{msg.senderId.name}</p>}
                                        <p>{msg.content}</p>
                                    </div>
