...
import {
    Sparkles, Clock, User,
    MapPin, X, Loader2, Calendar,
    Users, Briefcase, ChevronLeft,
    CheckCircle2, XCircle, Info, MessageSquare
} from 'lucide-react';
        ...
        <div className="flex bg-slate-50 min-h-screen font-outfit">
            ...
            <div className="flex-1 ml-64 p-10">
                <button 
                    ...
                    className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-colors mb-8"
                >
                ...
                <div className="max-w-5xl mx-auto">
                    {/* Hero Section */}
                    <div className="bg-white rounded-[3rem] overflow-hidden shadow-2xl shadow-slate-200/50 border border-slate-100 mb-10">
                        <div className="bg-gradient-to-br from-slate-900 to-indigo-900 p-12 text-white relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="flex flex-wrap items-center gap-8 text-indigo-100/80">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-bold uppercase tracking-widest">{trip.members?.length || 0} Travelers</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                                <div className="md:col-span-2 space-y-10">
                                    <div>
                                        ...
                                        <p className="text-slate-600 leading-relaxed font-medium bg-slate-50 p-8 rounded-[2rem] border border-slate-100/50">
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            ...
                                            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Pace</p>
                                                <p className="text-sm font-bold text-slate-800 uppercase tracking-tight">{trip.preferences.pace}</p>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            {trip.itinerary && trip.itinerary.length > 0 ? (
                                                trip.itinerary.map((day, idx) => (
                                                    <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-start gap-4">
                                                        <div className="w-10 h-10 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black shrink-0">
                                                            {idx + 1}
                                                        </div>
                                                        <div>
                                                            <h4 className="text-sm font-black uppercase tracking-tight mb-1">Day {idx + 1}</h4>
                                                            <p className="text-xs text-slate-500 font-medium leading-relaxed">{day.description}</p>
                                                        </div>
                                                    </div>
                                                ))
                                            ) : (
                                ...
                                <div className="space-y-8">
                                    <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-indigo-200/50">
                                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-6">Organizer Details</h3>
                                        <div className="flex items-center gap-4 mb-8">
                                            <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center text-2xl font-black border border-white/20 overflow-hidden">
                                                {trip.userId.avatarURL ? (
                                                    <img src={trip.userId.avatarURL} className="w-full h-full object-cover" />
                                                ) : (
                                                    trip.userId.name.charAt(0)
                                                )}
                                            </div>
                                            <div>
                                                <p className="text-lg font-black tracking-tight uppercase leading-tight">{trip.userId.name}</p>
                                                <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mt-1">Organizer</p>
                                            </div>
                                        </div>
                                        <div className="pt-6 border-t border-white/10 space-y-4">
                                            <button 
                                                ...
                                                disabled={responding}
                                                className="w-full py-4 bg-indigo-500 hover:bg-indigo-400 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-2 disabled:opacity-50"
                                            >
                                                {responding ? <Loader2 className="animate-spin" size={16} /> : <><CheckCircle2 size={16} /> Accept Assignment</>}
                                            </button>
                                            <button 
                                                onClick={() => setShowRejectModal(true)}
                                                className="w-full py-4 bg-white/10 hover:bg-white/20 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border border-white/10 flex items-center justify-center gap-2"
                                            >
                                    ...
                                    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl shadow-slate-200/50">
                                        ...
                                        <div className="flex items-baseline gap-2">
                                            <p className="text-4xl font-black text-slate-900">₹{((calcDays(trip.startDate, trip.endDate) ) * (authService.getCurrentUser()?.guideProfile?.hourlyRate || 0)).toLocaleString()}</p>
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl overflow-hidden p-8 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="mt-10 flex gap-4">
                            ...
                            <button 
                                ...
                                className="flex-[2] py-4 px-6 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-red-600 disabled:opacity-50 transition-all"
                            >
                                {responding ? <Loader2 className="animate-spin" size={16} /> : <>Decline Request <XCircle size={14} /></>}
                            </button>
                        </div>
                    </div>
                </div>
