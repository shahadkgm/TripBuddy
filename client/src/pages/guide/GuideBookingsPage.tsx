...
export const GuideBookingsPage = () => {
    ...
    const fetchBookings = async (pageNum: number = 1) => {
        ...
        try {
            ...
            console.log("DEBUG: Fetching bookings for guide ID:", currentUser.guideProfile._id, "page:", pageNum);
                ...
                <div className="relative">
                    ...
                    {loading ? (
                        <div className="py-20 flex flex-col items-center justify-center gap-4">
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Loading assignments...</p>
                        </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 text-center lg:text-left min-w-0">
                                            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 mb-3">
                                                <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm
                                                    ...
                                                    trip.status === 'ongoing' ? 'bg-blue-50 text-blue-600 border border-blue-100' : 
                                                    trip.status === 'planned' ? 'bg-amber-50 text-amber-600 border border-amber-100' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>
                                                    {trip.status}
                                                </span>
                                            ...
                                            <h4 className="text-2xl font-black text-slate-900 tracking-tighter uppercase truncate">{trip.title}</h4>
                                            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 mt-4">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100">
                                                        <MapPin size={14} className="text-indigo-500" />
                                                    </div>
                                                    <span className="text-xs font-bold text-slate-500">{trip.destination}</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100">
                                                        <User size={14} className="text-indigo-500" />
                                                    </div>
                                                    <span className="text-xs font-bold text-slate-500">Organizer: {typeof trip.userId === 'object' ? trip.userId.name : 'User'}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex lg:flex-col items-center lg:items-end gap-3 shrink-0">
                                            <div className="text-right hidden lg:block mb-2">
                                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-tight">Earnings</p>
                                                <p className="text-xl font-black text-indigo-600">₹{(calcDays(trip.startDate, trip.endDate) * (user?.guideProfile?.hourlyRate || 0)).toLocaleString()}</p>
                                            </div>
                                            <button className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200">
                        ...
                        <div className="bg-white rounded-[3rem] border-2 border-dashed border-slate-200 p-24 text-center">
                            ...
                            <h4 className="text-2xl font-black text-slate-900 tracking-tight">No bookings found</h4>
