            ...
            <div className="flex-1 ml-64 p-10">
                ...
                <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                        <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400">Payment History</h3>
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-black text-slate-400 uppercase mr-2">Filter By:</span>
                                ...
                                {loading ? (
                                    ...
                                    <tr key={trip._id} className="group hover:bg-slate-50/50 transition-colors">
                                        <td className="px-10 py-6">
                                            <p className="text-sm font-black text-slate-900 uppercase truncate max-w-[200px]">{trip.title}</p>
                                            <p className="text-[10px] font-bold text-slate-400 mt-1">{trip.destination}</p>
                                        </td>
                                        <td className="px-10 py-6 text-xs font-bold text-slate-500">
                                            {new Date(trip.endDate).toLocaleDateString()}
                                        </td>
                                        <td className="px-10 py-6">
                                            <span className="text-sm font-black text-slate-900">₹{(calcDays(trip.startDate, trip.endDate) * (user?.guideProfile?.hourlyRate || 0)).toLocaleString()}</span>
                                        </td>
                                        <td className="px-10 py-6">
