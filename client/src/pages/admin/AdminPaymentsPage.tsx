...
export const AdminPaymentsPage = () => {
  ...
  const [selectedPayment, setSelectedPayment] = useState<{ id: string; status: string } | null>(
    null
  );
  const limit = 3;
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await api.patch(`/api/admin/payments/${selectedPayment.id}/status`, {
      ...
      render: (payment: PaymentData) => (
        <div className="flex items-center gap-3 max-w-[200px]">
          {payment.userId?.avatarURL ? (
            <img
              src={
                payment.userId.avatarURL.startsWith('http')
                  ? payment.userId.avatarURL
                  : `${api.defaults.baseURL}/${payment.userId.avatarURL}`
              }
      ...
      render: (payment: PaymentData) => (
        <div className="max-w-[180px]">
          <div className="font-medium text-gray-800 text-sm break-words line-clamp-2">
            {payment.tripId?.title || 'Unknown Trip'}
          </div>
        ...
        <div className="flex flex-col items-center gap-1">
          <span
            className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
              payment.status === 'released' || payment.status === 'escrowed'
          ...
          {payment.status === 'refunded' && payment.refundReason && (
            <span
              className="text-[10px] text-purple-500 italic max-w-[120px] truncate"
      ...
      render: (payment: PaymentData) => (
        <div className="flex justify-end gap-2">
          {payment.status === 'escrowed' && (
            <button
              onClick={() => {
                setSelectedPayment({ id: payment._id, status: 'released' });
                setIsModalOpen(true);
              }}
          ...
          {(payment.status === 'escrowed' || payment.status === 'released') && (
            <button
              onClick={() => {
                setSelectedPayment({ id: payment._id, status: 'refunded' });
                setIsModalOpen(true);
              }}
          ...
          {payment.status === 'pending' && (
            <button
              onClick={() => {
                setSelectedPayment({ id: payment._id, status: 'failed' });
                setIsModalOpen(true);
              }}
