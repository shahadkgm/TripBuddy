...
const ReportManagementPage: React.FC = () => {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case 'dismissed': return 'bg-slate-50 text-slate-400 border-slate-100';
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm shadow-slate-100/50">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium mt-1">Protecting the community through verified moderation</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50">
                  ...
                  <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Violation Details</th>
                  ...
                  <tr>
                    <td colSpan={5} className="px-8 py-24 text-center">
                      <div className="max-w-xs mx-auto">
                        ...
                        <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Clear skies! No pending violations found.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-black text-slate-900 truncate tracking-tight">{report.reporterId?.name}</p>
                            <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">Reporter</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="max-w-xs">
                          <p className="text-sm font-black text-slate-800 tracking-tight">{report.reason}</p>
                          <p className="text-[10px] text-slate-400 font-medium line-clamp-1 mt-0.5">"{report.description}"</p>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <div className="relative">
                            <img 
                              src={report.targetId?.avatarURL || `https://ui-avatars.com/api/?name=${report.targetId?.name}`} 
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-black text-rose-600 truncate tracking-tight">{report.targetId?.name}</p>
                            <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">{report.targetType}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex justify-center">
                          <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border flex items-center gap-2 shadow-sm ${getStatusStyle(report.status)}`}>
          ...
          <div className="fixed inset-0 z-50 flex justify-end animate-in fade-in duration-300">
            ...
            <div className="relative w-full max-w-lg bg-white shadow-2xl h-full flex flex-col animate-in slide-in-from-right duration-500">
              <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 tracking-tighter">Report Details</h3>
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Reference: {selectedReport._id.slice(-8)}</p>
                </div>
                <button onClick={() => setSelectedReport(null)} className="p-3 hover:bg-slate-50 rounded-2xl text-slate-300 hover:text-slate-900 transition-all">
                  <XCircle size={24} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                <div className="space-y-4">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block px-1">Case Context</span>
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        <h4 className="text-lg font-black text-slate-900 tracking-tight">{selectedReport.reason}</h4>
                    ...
                    <div className="bg-white p-4 rounded-2xl border border-slate-100">
                      <p className="text-xs text-slate-600 font-medium italic leading-relaxed">"{selectedReport.description}"</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block px-1">Reporter</span>
                    <div className="bg-white p-4 rounded-3xl border border-slate-100 flex items-center gap-3">
                       <img 
                          src={selectedReport.reporterId?.avatarURL || `https://ui-avatars.com/api/?name=${selectedReport.reporterId?.name}`} 
                          className="w-10 h-10 rounded-xl object-cover shadow-sm"
                          alt="" 
                        />
                        <div className="min-w-0">
                          <p className="text-sm font-black text-slate-800 truncate tracking-tight">{selectedReport.reporterId?.name}</p>
                          <p className="text-[9px] text-slate-400 font-bold truncate uppercase">{selectedReport.reporterId?.email}</p>
                        </div>
                    </div>
                  </div>
                  <div className="space-y-3">
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block px-1">Target User</span>
                     <div className="bg-rose-50/30 p-4 rounded-3xl border border-rose-100 flex items-center gap-3">
                       <img 
                          src={selectedReport.targetId?.avatarURL || `https://ui-avatars.com/api/?name=${selectedReport.targetId?.name}`} 
                          className="w-10 h-10 rounded-xl object-cover shadow-sm ring-2 ring-white"
                          alt="" 
                        />
                        <div className="min-w-0">
                          <p className="text-sm font-black text-rose-600 truncate tracking-tight">{selectedReport.targetId?.name}</p>
                          <p className="text-[9px] text-slate-400 font-bold truncate uppercase">{selectedReport.targetType}</p>
                        </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                   <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block px-1">Associated Trip</span>
                   <div className="bg-white p-6 rounded-3xl border border-slate-100 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-black text-slate-900 tracking-tight">{selectedReport.tripId?.title}</p>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{selectedReport.tripId?.destination}</p>
                        </div>
                      </div>
              ...
              <div className="p-8 border-t border-slate-100 bg-slate-50/50">
                {selectedReport.status === 'pending' ? (
                  ...
                  <div className={`p-5 rounded-2xl border text-center font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 ${getStatusStyle(selectedReport.status)}`}>
                     {selectedReport.status === 'resolved' ? <CheckCircle size={18}/> : <XCircle size={18}/>}
