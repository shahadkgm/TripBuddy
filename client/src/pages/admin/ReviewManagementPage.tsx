        ...
        <div className="min-h-screen bg-slate-50 p-6 md:p-10">
            <div className="max-w-7xl mx-auto">
                <header className="mb-10 flex justify-between items-center">
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-2">Review Management</h1>
                        <p className="text-slate-500 font-medium">Monitor and manage traveler feedback across the platform.</p>
                    </div>
                </header>
                ...
                {reviews.length === 0 ? (
                    <div className="bg-white rounded-3xl p-16 text-center border-2 border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500">Reviews will appear here once travelers complete their trips.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {reviews.map((review) => (
                            <div key={review._id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 group">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="w-12 h-12 bg-slate-100 rounded-2xl overflow-hidden shadow-inner">
                                            {review.reviewerId.avatarURL ? (
                                                <img src={review.reviewerId.avatarURL} alt={review.reviewerId.name} className="w-full h-full object-cover" />
                                            ) : (
                                        ...
                                        <div>
                                            <h4 className="font-bold text-slate-800 line-clamp-1">{review.reviewerId.name}</h4>
