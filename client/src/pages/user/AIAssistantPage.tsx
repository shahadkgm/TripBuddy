...
const AIAssistantPage = () => {
    const [messages, setMessages] = useState<Message[]>([
        {
            role: 'assistant',
            content: "👋 Hi! I can help you find places, plan trips, and suggest best destinations. Where would you like to go?"
        }
    ]);
            ...
            <div className="bg-white w-full max-w-4xl rounded-[30px] shadow-2xl overflow-hidden flex flex-col h-[85vh] relative border border-white/20">
                    ...
                    {messages.map((m, i) => (
                        ...
                        >
                            <div className={`flex gap-3 max-w-[85%] ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${m.role === 'user' ? 'bg-indigo-100 text-indigo-600' : 'bg-indigo-600 text-white shadow-md'
                                    }`}>
                        ...
                        <div className="flex justify-start animate-in fade-in duration-300">
                            <div className="flex gap-3 max-w-[85%]">
                                ...
                                <div className="bg-white p-4 rounded-2xl rounded-tl-none border border-slate-100 shadow-sm flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-gray-500 font-medium">Trip Buddy is thinking...</span>
                                </div>
                            </div>
                        </div>
                    )}
                ...
                {!isLoading && messages.length === 1 && (
                    <div className="px-6 py-2 flex flex-wrap gap-2 bg-slate-50/50">
                        {["Suggest a trip to Bali", "Plan 3 days in Paris", "Best beaches in Goa"].map((suggestion) => (
                            <button
                                key={suggestion}
                                onClick={() => {
                                    setInput(suggestion);
                ...
                <div className="p-4 md:p-6 bg-white border-t border-slate-100">
                    <form onSubmit={handleSend} className="relative flex items-center bg-slate-50 rounded-2xl p-1 shadow-inner border border-slate-200 focus-within:border-indigo-300 transition-colors">
                        <input
                            type="text"
                            value={input}
