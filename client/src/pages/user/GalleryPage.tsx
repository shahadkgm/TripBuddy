            ...
            <main className="max-w-3xl mx-auto px-6 py-12">
                {loading ? (
                    ...
                    <div className="space-y-12">
                        {posts.map((post) => (
                            <div key={post._id} className="bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-xl shadow-slate-200/40 group transition-all hover:shadow-2xl">
                                {/* Post User Header */}
                    ...
                    <div className="text-center py-40 flex flex-col items-center gap-8">
                        <div className="w-40 h-40 bg-white rounded-full flex items-center justify-center border border-slate-100 shadow-xl relative group">
                            ...
                            <ImageIcon size={64} className="text-slate-200 group-hover:text-indigo-200 transition-colors" />
                        </div>
                        <div className="space-y-2">
                            <p className="font-black text-2xl text-slate-400 uppercase tracking-tighter">Gallery is Quiet</p>
                            <p className="text-slate-400 font-medium max-w-xs mx-auto">Click the button above to share your first travel story!</p>
                        </div>
                    </div>
                )}
            </main>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
                    ...
                    <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl relative z-10 animate-in zoom-in slide-in-from-bottom-12 duration-500">
                        ...
                        <div className="mb-10">
                            ...
                            <p className="text-slate-400 font-medium mt-1">Post a beautiful moment from your trip.</p>
                        </div>
                        ...
                        <form onSubmit={handlePostSubmit} className="space-y-8">
                            ...
                            >
                                {uploadData.image ? (
                                    <div className="w-full h-full">
                                        <img src={uploadData.image} alt="Preview" className="w-full h-full object-cover" />
                                        <div className="absolute inset-0 bg-slate-900/60 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                            <p className="text-white font-black text-xs uppercase tracking-widest bg-white/10 px-6 py-3 rounded-full backdrop-blur-md border border-white/20">Change Photo</p>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                        {isUploading ? (
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <p className="text-indigo-600 font-black text-[10px] uppercase tracking-widest">Uploading...</p>
                                            </div>
                                        ) : (
                                            <>
                                                ...
                                                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">JPEG, PNG, WebP up to 5MB</p>
                                            </>
                                        )}
                                    </>
                                )}
                            ...
                            <div className="space-y-5">
                                <div className="relative">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 mb-2">Trip Destination Context</label>
                                    <div className="relative">
                                ...
                                <div className="relative">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 mb-2">Memory / Bio</label>
                                    <textarea
                            ...
                            >
                                {isUploading ? "Posting Story..." : (
                                    <span className="flex items-center justify-center gap-2">
                                        Post Memory <Plus size={16} className="group-hover:rotate-90 transition-transform" />
                                    </span>
                                )}
                            </button>
                        </form>
