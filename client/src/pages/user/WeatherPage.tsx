...
export const WeatherPage = () => {
    ...
    const renderIcon = (iconName: string) => {
        const props = { className: "w-16 h-16 transition-transform duration-700 group-hover:scale-110", strokeWidth: 1.5 };
        switch (iconName) {
            case 'Sun': return <Sun {...props} className={`${props.className} text-amber-400`} />;
            case 'Cloud': return <Cloud {...props} className={`${props.className} text-slate-400`} />;
            case 'CloudSun': return <CloudSun {...props} className={`${props.className} text-indigo-400`} />;
            case 'CloudRain': return <CloudRain {...props} className={`${props.className} text-blue-400`} />;
            case 'CloudLightning': return <CloudLightning {...props} className={`${props.className} text-indigo-600`} />;
            case 'CloudFog': return <CloudFog {...props} className={`${props.className} text-slate-300`} />;
            case 'CloudDrizzle': return <CloudDrizzle {...props} className={`${props.className} text-blue-300`} />;
            default: return <Sun {...props} className={`${props.className} text-amber-400`} />;
        }
    };
        ...
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-outfit">
            ...
            <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-12 flex flex-col items-center">
                <div className="text-center mb-12">
                    <h1 className="text-5xl font-black text-slate-900 mb-3 tracking-tighter">Weather Forecast</h1>
                    <p className="text-slate-400 font-medium">Plan your travel according to the skies.</p>
                </div>

                {/* Search Bar - Custom Trip Buddy Style */}
                <form onSubmit={handleSearch} className="w-full max-w-2xl flex flex-col md:flex-row gap-3 mb-16">
                        ...
                        <div className="bg-white rounded-[3rem] p-12 border border-slate-100 shadow-2xl relative overflow-hidden transition-all duration-500 group-hover:-translate-y-2 group-hover:border-indigo-200">
                            ...
                            <div className="relative z-10">
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
                                    <div>
                                        <h2 className="text-4xl font-black text-slate-900 tracking-tighter">{weather.city}</h2>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
                                            <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px]">{weather.date}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
                                    ...
                                    <div className="flex flex-row lg:flex-col gap-4 w-full lg:w-48">
                                        <div className="flex-1 bg-indigo-50/50 p-5 rounded-3xl border border-indigo-100/50 group/item hover:bg-indigo-600 transition-all duration-300">
                                            <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1 group-hover/item:text-indigo-200">UV Index</p>
                                            <p className="text-2xl font-black text-indigo-700 group-hover/item:text-white">{weather.uvIndex}</p>
                                        </div>
                                        <div className="flex-1 bg-slate-50 p-5 rounded-3xl border border-slate-100 group/item hover:bg-slate-900 transition-all duration-300">
                                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 group-hover/item:text-slate-500">Humidity</p>
                                            <p className="text-2xl font-black text-slate-800 group-hover/item:text-white">{weather.humidity}%</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-12 pt-10 border-t border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-6">
                                    <StatBox icon={<Thermometer size={14} />} label="Feels Like" value={`${weather.feelsLike}°`} color="hover:text-indigo-600" />
                                    <StatBox icon={<Wind size={14} />} label="Wind" value={`${weather.windSpeed} km/h`} color="hover:text-blue-500" />
                                    <StatBox icon={<Droplets size={14} />} label="Min Temp" value={`${weather.minTemp}°`} color="hover:text-cyan-500" />
                                    <StatBox icon={<Sun size={14} />} label="Max Temp" value={`${weather.maxTemp}°`} color="hover:text-amber-500" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {!weather && !loading && (
                    <div className="text-center py-24 flex flex-col items-center gap-6 animate-pulse">
                        ...
                        <div>
                            <p className="font-black text-xl text-slate-300 uppercase tracking-widest">Sky Scan Inactive</p>
                            <p className="text-slate-400 text-sm mt-1">Enter a destination to view local weather.</p>
                        </div>
                    </div>
                )}
            </main>
...
const StatBox = ({ icon, label, value, color }: { icon: any, label: string, value: string, color: string }) => (
    <div className={`p-5 bg-white border border-slate-100 rounded-[2rem] text-center transition-all hover:shadow-xl hover:shadow-slate-200/50 cursor-default group/stat ${color}`}>
        <div className="flex items-center justify-center gap-2 mb-2">
            <span className="text-slate-300 group-hover/stat:text-current transition-colors">{icon}</span>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{label}</p>
        </div>
        <p className="text-lg font-black text-slate-900 group-hover/stat:scale-110 transition-transform">{value}</p>
    </div>
);
