...
export interface ITripRepository extends IBaseRepository<ITripDocument, CreateTripDTO> {
    findByUserId(userId: string, page: number, limit: number): Promise<{ trips: ITripDocument[], total: number }>;
    ...
    findByGuideId(guideId: string, page: number, limit: number): Promise<{ trips: ITripDocument[], total: number }>;
}
