...
export class ReviewService implements IReviewService {
    ...
    private _getId(field: Types.ObjectId | string | Record<string, unknown> | null | undefined): string {
        if (!field) return '';
    ...
    async createReview(data: {
        ...
        const isMember = trip.members.some(m => this._getId(m as unknown as Record<string, unknown>) === data.reviewerId);
        const isOrganizer = this._getId(trip.userId as unknown as Record<string, unknown>) === data.reviewerId;
        ...
        const existingReview = await this._reviewRepository.findOne(query as FilterQuery<IReviewDocument>);
        if (existingReview) {
