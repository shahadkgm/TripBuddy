...
export class GalleryService implements IGalleryService {
    ...
    async getUserGallery(targetUserId: string, currentUserId: string): Promise<IGalleryPost[]> {
        ...
        const isConnected = connections.some(conn => {
            ...
            return conn.status === 'accepted' && (senderId === currentUserId || receiverId === currentUserId);
        });

        if (!isConnected) {
            throw new Error('Unauthorized: You can only view galleries of people you are connected with.');
        }
