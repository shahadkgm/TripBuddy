...
const fileFilter: multer.Options['fileFilter'] = (req, file, cb) => {
  ...
  logger.info(
    `File upload check: originalname="${file.originalname}", mimetype="${mimetype}", extension="${extension}", isExtensionValid=${isExtensionValid}, isMimeTypeValid=${isMimeTypeValid}`
  );
  ...
  if (isExtensionValid && isMimeTypeValid) {
    ...
    logger.error(
      `File upload rejected: originalname="${file.originalname}", mimetype="${mimetype}", extension="${extension}"`
    );
